...
<template>
  <div class="p-4">
    ...
    <div class="flex flex-wrap gap-4 justify-left pb-4">
      <IconButton @click="createRepo" :icon="FolderAddOutlined" label="Tambah Repository" color="blue" />
    </div>
    ...
    <ul v-else>
      <li v-for="repo in repos" :key="repo.id" class="flex justify-between items-center my-2 p-3 bg-gray-100 rounded shadow-md hover:bg-blue-100 cursor-pointer" @click="router.push(`/${repo.name}`)">
        ...
        <div class="flex items-center ml-auto">
          <button class="flex bg-red-500 text-white p-3 rounded-full hover:bg-red-600 cursor-pointer" @click.stop="deleteRepo(repo.name)">
            <DeleteOutlined />
          </button>
        </div>
      </li>
